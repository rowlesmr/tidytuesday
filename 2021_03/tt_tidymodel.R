...
art_rec <- recipe(year ~ medium, data = art_train) %>% 
  #update_role(artist, new_role = "ID") # can then use -has.role("ID") in a later step in the recipe to ignore these columns -- see Julia's astronaut video at ~17:00
...
collect_predictions(art_final) %>% 
  ...
  geom_abline(lty = 2, colour = "red", size = 2)+
...
missclassified %>% unnest_tokens(word,medium) %>% 
  count(word, sort =T)
